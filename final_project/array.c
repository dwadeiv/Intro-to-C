...
int printArray(array const * a, printFn f) { /*This passes likes takes the addres of a print fucntion from a user and the array */
  if(!a)                    /*makes sure that a is not null*/
    return -1;
  if(!f)
    return -1;
  for (int i = 0; i  < (int) a->inUse; i++){         /*iterates the loop for the number of elements in the array */
    f(a->data[i]);                                 /*passes the value of an element in the arary into the given print function */
  }
  return 0;
}
