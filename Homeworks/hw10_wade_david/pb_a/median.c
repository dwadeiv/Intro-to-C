...
int main(void) {
  ...
  while ((ret = scanf("%d", &num)) != EOF) {
    /* Checks for valid arguments then creates enough space in the heap for the needed array through the use of realloc */
    if(ret != 1) {
      printf("Only integer arguments are allowed. Median undefined.\n");
      return -1;
    }
    if(ret == 1) {
      if(cnt == size) {
