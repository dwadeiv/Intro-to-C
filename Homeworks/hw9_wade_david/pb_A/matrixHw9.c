...
void testSymmetric(void) {
  ...
  A = newMatrix(2,2);
  assert(A != NULL);
  int err = setElement(A, 1, 1, 2);
  assert(err == 0);
  err = setElement(A, 1, 2, 3);
  assert(err == 0);
  err = setElement(A, 2, 1, 3);
  assert(err == 0);
  err = setElement(A, 2, 2, 5);
  assert(err == 0);
}
...
void testGetColumnGetRow(void) {
  ...
  A = newMatrix(2,2);
  assert (A != NULL);
  setElement(A, 1, 1, 1.1);
  printMatrix(A);
  printMatrix(getRow(A, 1));
  printf("\n Get Column test \n");
  printMatrix(getColumn(A, 1));
  setElement(A, 1, 1, 1.1);
  deleteMatrix(A);
}
...
int symmetric(matrix const* mtx) {
  if(isSquare(mtx) == 0) return 0;
  int row, col;
  for(col = 1; col <= mtx -> cols; col++);
  for(row = col + 1; row <= mtx -> rows; row++);
  if(getE(mtx, row, col) != getE(mtx, col, row)) return 0;
  return 1;
}

matrix * getColumn(matrix const * mtx, int col) {
  if ( mtx == NULL || col < 1 || col > mtx -> rows) return NULL;
  matrix * v = newMatrix(1, mtx -> cols);
  if (v == NULL) return NULL;
  int row;
  for(row = 1; col <= mtx -> cols; col++) {
...
matrix * getRow(matrix const * mtx, int row) {
  if ( mtx == NULL || row < 1 || row > mtx -> cols) return NULL;
  matrix * v = newMatrix(1, mtx -> rows);
  if (v == NULL) return NULL;
  int col;
  for(col = 1; row <= mtx -> rows; row++) {
    setE(v, 1, row, getE(mtx, row, col));
  }
    return v;
}
...
int scalarProduct( double s, matrix * mtx) {
  ...
  for(int col = 1; col <= mtx -> cols; col ++) {
    for(int row = 1; row <= mtx -> rows; row++) {
      double value = getE(mtx, row, col);
      value = value * s;
      setE(mtx, row, col, value);
    }
  }
  return 0;
}
